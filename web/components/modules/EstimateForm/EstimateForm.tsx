...
const reducer = (state: EstimateState, action: Actions) => {
  ...
  if (action.type === 'add') {
    return set(
      ...
      view(lensPath(action.path || []), state).concat([{ id: _newId(), ...newRecord }]),
      state,
    ) as EstimateState;
...
function EstimateRow(props: SubEstimateProps) {
  const addSub = React.useCallback(
    () => {
      props.dispatch({ type: 'add', path: [...props.path, props.orderId, 'sub'] })
    },
    [props.dispatch],
  );
...
const EstimateForm: React.FC<EstimateFormPublicProps> = (
  ...
  return (
    <Box>
      {estimates.map(
        (est, k) => (
          <EstimateRow key={est.id} id={est.id} orderId={k} estimate={est} path={[]} dispatch={dispatch}/>
        ),
      )}
      <Button variant="outline" onClick={addRow}>+</Button>
    </Box>
