...
export default function useApiEndpoint<T>(
  endpoint: string,
  ...
  const apiFn = () =>
    ["GET", "DELETE"].includes(config.method || "GET")
      ...
      : handleApiResponse(
          axiosFn(url, config.payload || {}, { ...axiosDefaultConfig }),
        );

  const [state, getter] = useAsyncData(apiFn, {
    data: ifUndefined(config.initialData, null),
    message: null,
  });
